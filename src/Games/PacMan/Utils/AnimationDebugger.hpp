...
class AnimationDebugger {
 public:
    static void dumpAssetKeys(const std::map<std::string,
        DrawableComponent>& assets) {
        std::ofstream file("pacman_assets.log");
        if (!file.is_open()) {
            std::cerr << "Failed to open debug log file" << std::endl;
            return;
        }
        file << "Available asset keys ("
            << assets.size() << " total):" << std::endl;
        ...
        file << "\n=== PACMAN KEYS (" << pacmanKeys.size()
            << ") ===" << std::endl;
        for (const auto& key : pacmanKeys) {
            file << key << std::endl;
        }
        file << "\n=== GHOST KEYS (" << ghostKeys.size()
            << ") ===" << std::endl;
    ...
    static std::string getDirectionalSpriteAlternative(
        const PacmanComponent* pacman) {
        ...
        int frame = pacman->getAnimationFrame() - 1;
